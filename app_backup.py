...
# Configuración
JWT_SECRET = os.getenv('JWT_SECRET', 'mi-clave-            print("3. 📜 Ver historial de emails")
            print("4. 👋 Salir")creta-super-segura')
...
def registrar_email_enviado(user_id, username, from_email, to_email, to_name, subject, html_content, provider_usado, estado, error_mensaje=None):
...
def enviar_email_interactivo(usuario_actual):
    ...
    try:
        ...
        error_mensaje = None if resultado.get('success') else resultado.get('error', resultado.get('response_text'))
...
def mostrar_historial_emails(usuario_actual):
    ...
    try:
        ...
        for i, email in enumerate(historial, 1):
            ...
            print(f"   📧 Para: {email['to_email']} ({email['to_name']})")
            print(f"   📌 Asunto: {email['subject']}")
            print(f"   🔧 Proveedor: {email['provider_usado']}")
            
            # Mostrar error si existe
            if email['estado'] == 'fallido' and email.get('error_mensaje'):
                print(f"   ❌ Error: {email['error_mensaje']}")
